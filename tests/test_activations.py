...
from onmt.modules.activations import SparsemaxFunction, Sparsemax, ConstrainedSparsemaxFunction, ConstrainedSparsemax
...
def test_active_fns():
    ...
    z = Variable(torch.randn(batch_size, n).double().to(device), requires_grad=True)
    p = Sparsemax()(z)
    dp = torch.randn(batch_size, n).double().to(device)
    p.backward(dp)
    dz = z.grad.data
    dz = dz.cpu().numpy()
    print(dp.cpu().numpy())
    ...
    u = Variable(1. / n + 1. / n * torch.rand(batch_size, n).double().to(device),
                 requires_grad=True)
    ...
    all_ones = torch.ones(batch_size, n).double().to(device)
    assert torch.equal(all_ones, torch.le(p, u).data.double().to(device)), \
