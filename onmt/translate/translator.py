...
class APETranslator(Translator):
    """
    Uses an APE model to translate a batch of sentences.
    ...
    def translate(self,
        ...
        Args:
            ...
            src_data_iter (iterator): an interator generating source data
                e.g. it may be a list or an openned file
            mt_path (str): filepath of mt data
            mt_data_iter (iterator): an interator generating mt data
            tgt_path (str): filepath of target data
            tgt_data_iter (iterator): an interator generating target data
            batch_size (int): size of examples per mini-batch
            attn_debug (bool): enables the attention logging
        ...
        Returns:
            ...
            * all_predictions is a list of `batch_size` lists
                of `n_best` predictions
        """
        assert mt_data_iter is not None or mt_path is not None
    ...
    def _fast_translate_batch(self,
        ...
        mt = inputters.make_features(batch, 'mt', data.data_type)
        ...
        if char_mt is not None:
            sorted_char_mt, _, _ = self.model.sort_sentences(char_mt,
                                                             mt_lengths)
        else:
            sorted_char_mt = None
        ...
        dec_states = self.model.decoder.init_decoder_state(
                            ...
                            enc_states_src, enc_states_mt)

        # Tile states and memory beam_size times.
        dec_states.map_batch_fn(
            lambda state, dim: tile(state, beam_size, dim=dim))

        memory_bank_src = tile(memory_bank_src, beam_size, dim=1)
        memory_lengths_src = tile(src_lengths, beam_size)
        memory_bank_mt = tile(memory_bank_mt, beam_size, dim=1)
        memory_lengths_mt = tile(mt_lengths, beam_size)

        batch_offset = torch.arange(
            batch_size, dtype=torch.long, device=memory_bank_src.device)
        beam_offset = torch.arange(
            0,
            batch_size * beam_size,
            step=beam_size,
        ...
        topk_log_probs = (
            torch.tensor([0.0] + [float("-inf")] * (beam_size - 1),
                         device=memory_bank_src.device).repeat(batch_size))
        ...
        for step in range(max_length):
                ...
                for i in finished:
                    ...
                    for n in range(n_best):
                        ...
                        if attention is None:
                            ...
                            results["attention"][b].append(
                                attention[:, i, n, :memory_lengths_mt[i]])
                non_finished = end_condition.eq(0).nonzero().view(-1)
                # If all sentences are translated, no need to go further.
                if len(non_finished) == 0:
                    break
            ...
            memory_bank_src = memory_bank_src.index_select(1, select_indices)
            memory_lengths_src = memory_lengths_src.index_select(
                                            0, select_indices)
            memory_bank_mt = memory_bank_mt.index_select(1, select_indices)
            memory_lengths_mt = memory_lengths_mt.index_select(
                                            0, select_indices)
            dec_states.map_batch_fn(
                lambda state, dim: state.index_select(dim, select_indices))
    ...
    def _translate_batch(self, batch, data):
        ...
        if src_lengths is None:
            src_lengths = torch.Tensor(batch_size).type_as(
                                    memory_bank_src.data)\
                                    .long()\
                                    .fill_(memory_bank_src.size(0))

        if mt_lengths is None:
            mt_lengths = torch.Tensor(batch_size).type_as(
                                    memory_bank_mt.data)\
                                    .long()\
                                    .fill_(memory_bank_mt.size(0))

        # (2) Repeat src objects `beam_size` times.
        mt_map = rvar(batch.mt_map.data) \
            if data_type == 'text' and self.copy_attn else None
        memory_bank_src = rvar(memory_bank_src.data)
        memory_lengths_src = src_lengths.repeat(beam_size)
        memory_bank_mt = rvar(memory_bank_mt.data)
        ...
        for i in range(self.max_length):
            ...
            if not self.copy_attn:
                ...
                out = data.collapse_copy_scores(
                    unbottle(out.data),
                    batch, self.fields["tgt"].vocab, data.mt_vocabs)
                # beam x tgt_vocab
                out = out.log()
                beam_attn = unbottle(attn["copy"])
            ...
            for j, b in enumerate(beam):
                b.advance(out[:, j],
                          beam_attn.data[:, j, :memory_lengths_src[j]])
                dec_states.beam_update(j, b.get_current_origin(), beam_size)
    ...
    def _run_target(self, batch, data):
        ...
        mt = inputters.make_features(batch, 'mt', data_type)
        _, mt_lengths = batch.mt
        ...
        dec_out, _, _ = self.model.decoder(
                ...
                memory_bank_src, memory_bank_mt,
                dec_states,
                memory_lengths_src=src_lengths,
                memory_lengths_mt=mt_lengths)
